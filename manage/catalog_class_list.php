...
$listUrl	= "catalog_class_list.php?page=$page";
$editUrl		= "catalog_class_edit.php";
...
{
	//是否有内容
	if ($db->getCount("catalog", "class_id='$id'") > 0)
	{
		$db->close();
		info("分类下有信息，请先删除信息！");
	}

	$sql = "delete from catalog_class where id='$id'";
	$rst = $db->query($sql);
	if ($rst)
	{
		$db->query("commit");
	...
	<div class="page-content-wrapper">
		...
		<div class="page-content">
			<ul class="page-breadcrumb breadcrumb">
				...
				<li><i class="fa fa-angle-right"></i><span class="active">栏目分类管理</span></li>
			</ul>
			<div class="row">
				<div class="col-md-12">
						...
						<form >
							<table class="table table-striped table-bordered table-hover table-checkable order-column dataTable no-footer" id="sample_1">
								...
								<tbody>
									...
									//总记录数
									$sql = "select count(*) as cnt from catalog_class";
									$rst = $db->query($sql);
									...
									$sql = "select id, sortnum, name, add_deny, delete_deny from catalog_class order by sortnum asc";
