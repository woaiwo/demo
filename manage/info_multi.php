...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	elseif ($operation == "copy") // 复制
	{
		...
		// 	$sql = "insert into info(id, sortnum, title, admin_id, class_id, author, source, website, pic, pic2, annex, keyword, intro, content, files, views, create_time, modify_time, state, price) values(". ($db->getMax("info", "id") + 1) .", ". ($db->getMax("info", "sortnum", "class_id='$end_class'") + 10) .", '$title', $session_admin_id, '$end_class', '$author', '$source', '$website', '$pic', '$pic2', '$annex', '$keyword', '$intro', '$content', '$files', 0, '$create_time', '$now', $state, '$price')";
		// 	$rst = $db->query($sql);
		// }
