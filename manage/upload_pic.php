...
$widthName	= htmlspecialchars(trim($_GET["widthName"]));
$heightName	= htmlspecialchars(trim($_GET["heightName"]));
$action     = htmlspecialchars(trim($_GET["action"]));

if($tableName=='info_class'){
...
$baseURL	= "?id=$id&tableName=" . $tableName . "&idName=" . $idName . "&picName=" . $picName . "&widthName=" . $widthName . "&heightName=" . $heightName . "";
...
if ($action == "del"){
	...
	{
		$sql = "update ".$tableName." set ".$picName." ='', ".$widthName."=0, ".$heightName."=0 where ".$idName."=$id";
	}else{
...
<script language="javascript">
	function check()
	{
		...
		obj.action =  "?id=<?=$id?>&tableName=<?=$tableName?>&idName=<?=$idName?>&picName=<?=$picName?>&widthName=<?=$widthName?>&heightName=<?=$heightName?>"
	...
	function del()
	{
		...
		{
			var obj		= document.editForm;
			window.location = "?id=<?=$id?>&tableName=<?=$tableName?>&idName=<?=$idName?>&picName=<?=$picName?>&widthName=<?=$widthName?>&heightName=<?=$heightName?>&action=del";
		}
	}
	...
	function returnPic()
	{
		if (window.opener)
		{
			var picLinkObj = window.opener.document.getElementById("pic<?=$id?>");
			if ('<?=$fileName?>'!='')
			{
				picLinkObj.innerHTML = "<font color='#FF6600'>有</font>";
				picLinkObj.setAttribute("picUrl", "<?=UPLOAD_PATH_FOR_ADMIN.$fileName?>");
				window.opener.BindPic();
			}
			else
			{
				picLinkObj.innerHTML = "上传";
				picLinkObj.setAttribute("picUrl", "");
				window.opener.BindPic();
			}
		}
		return;
