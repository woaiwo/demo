...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	$pic			= uploadImg($pic_file, "gif,jpg,png,swf,flv,wmv,mp4,rar,zip,xlsx,xls,doc,docx,ppt,pdf",$config_waterpic,$config_waterpos,$config_iswatermark);
...
<script language="javascript">
	function check()
	{
		...
		if (file_ext != 'jpg' && file_ext != 'jpeg' && file_ext != 'gif' && file_ext != 'png' && file_ext != 'swf' && file_ext != 'flv' && file_ext != 'wmv' && file_ext != 'mp4' && file_ext != 'rar' && file_ext != 'zip' && file_ext != 'xlsx' && file_ext != 'xls' && file_ext != 'doc' && file_ext != 'docx' && file_ext != 'ppt' && file_ext != 'pdf')
		{
			alert('上传文件必须是 JPG、JPEG、GIF、PNG、SWF、FLV、WMV、MP4、RAR、ZIP、XLSX、XLS、DOC、DOCX、PPT、PDF 格式！');
			return false;
		}
	...
	function returnPic()
	{
		if (window.opener)
		{
			var picLinkObj = window.opener.document.getElementById("annex<?=$id?>");
			if ('<?=$fileName?>'!='')
			{
		...
		<form name="editForm" method="post" enctype="multipart/form-data" onSubmit="return check(this);">
			<table class="editTable">
				...
				<tr>
					...
					<td>
						仅限 Jpg、Jpeg、Gif、Png、Swf、Flv、Wmv、Mp4、Rar、Zip、Xlsx、Xls、Doc、Docx、Ppt、Pdf 格式的图片。
					</td>
				</tr>
				<tr>
