...
if (empty($class_id))
{
	$sql = "select id, name, add_deny, delete_deny, hasPic1 from banner_class order by sortnum asc limit 1";
}
else
{
	$sql = "select id, name, add_deny, delete_deny, hasPic1 from banner_class where id=$class_id";
}
$rst = $db->query($sql);
...
if ($id != "")
{
	$sql = " select pic, pic1 from banner where id=$id";
	$rst = $db->query($sql);
	if ($row = $db->fetch_array($rst))
	{
		$pic 	= $row["pic"];
								...
								<table class="table table-striped table-bordered table-hover table-checkable order-column dataTable no-footer" id="sample_1">
									<thead>
										<tr>
						                    ...
						                    <th class="text-center hidden-xs">缩略图</th>
						                    <?
						                    	if ($class_hasPic1==1){
						                    ?>
											<th class="text-center hidden-xs">附件图</th>
											<?
												}
											?>
									...
									<tbody>
										...
										//列表
										$sql = "select id, class_id, sortnum, title, url, pic, pic1, width, height, state from banner where class_id=$class_id order by sortnum desc";
						                ...
						                <tr class="<?if($i%2==1){echo "odd";}else{echo "even";}?>" >
											...
											<th class="text-center hidden-xs">
												<?
	                                                if (!empty($row["pic"])){
	                                            ?>
	                                                <a id="pic<?=$row["id"]?>" href="javascript:banner('<?=$row["id"]?>', 'banner', 'id', 'pic', '', '')" class="picView"><font color='#FF6600'>有</font></a>
	                                            <?
	                                                }else{
	                                            ?>
	                                                <a id="pic<?=$row["id"]?>" href="javascript:banner('<?=$row["id"]?>', 'banner', 'id', 'pic', '', '')" class="picView">上传</a>
	                                            <?
	                                                }
	                                            ?>
											</th>
											...
											<th class="text-center hidden-xs">
												<?
	                                                if (!empty($row["pic1"])){
	                                            ?>
	                                                <a id="pic1<?=$row["id"]?>" href="javascript:banner1('<?=$row["id"]?>', 'banner', 'id', 'pic1', '', '')"><font color='#FF6600'>有</font></a>
	                                            <?
	                                                }else{
	                                            ?>
	                                                <a id="pic1<?=$row["id"]?>" href="javascript:banner1('<?=$row["id"]?>', 'banner', 'id', 'pic1', '', '')">上传</a>
	                                            <?
	                                                }
	                                            ?>
											</th>
