...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	$hasPic1		= (int)$_POST["hasPic1"];
	$hasCon			= (int)$_POST["hasCon"];

	if (empty($id))
	{
		$id = $db->getMax("banner_class", "id") + 1;
		$sql = "insert into banner_class(id, sortnum, name, add_deny, delete_deny, hasPic1, hasCon) values('$id', $sortnum, '$name', $add_deny, $delete_deny, $hasPic1, $hasCon)";
	}
	else
	{
		$sql = "update banner_class set sortnum=$sortnum, name='$name', add_deny=$add_deny, delete_deny=$delete_deny, hasPic1=$hasPic1, hasCon=$hasCon where id='$id'";
	}
	$rst = $db->query($sql);
	$db->close();
...
else
{
	...
	{
		$sql = "select id, sortnum, name, add_deny, delete_deny, hasPic1, hasCon from banner_class where id='$id'";
		$rst = $db->query($sql);
	...
	<div class="page-content-wrapper">
		...
		<div class="page-content">
			<ul class="page-breadcrumb breadcrumb">
				...
				<li><i class="fa fa-angle-right"></i>Banner分类管理</li>
			</ul>
			<div class="row">
				<div class="col-md-12">
 							...
 							<form class="form" name="editForm" method="post" onSubmit="return check(this);">
								<div class="form-body">
									...
									<div class="form-group">
										<div class="row">
											<div class="col-md-6">
												<label class="control-label">附件图片</label>
												<div class="checkbox-list">
	                                                <label class="checkbox-inline">
	                                               		<input type="radio" name="hasPic1" value="1" <?if ($hasPic1 == 1) echo "checked"?>> 有
	                                                </label>
	                                                <label class="checkbox-inline">
	                                               		<input type="radio" name="hasPic1" value="0" <?if ($hasPic1 == 0) echo "checked"?>> 无
	                                                </label>
	                                            </div>
											</div>
											<div class="col-md-6">
												<label class="control-label">文本描述</label>
												<div class="checkbox-list">
													<label class="checkbox-inline">
														<input type="radio" name="hasCon" value="1" <?if ($hasCon == 1) echo "checked"?>> 有
													</label>
													<label class="checkbox-inline">
														<input type="radio" name="hasCon" value="0" <?if ($hasCon == 0) echo "checked"?>> 无
													</label>
												</div>
											</div>
										</div>
